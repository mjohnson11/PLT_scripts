...
# Reading and adding old by-hand environment bc calls
ebc_file = './LT_bc_tp_exclusion/Environment_BC_calls_Feb_2018_simple.csv'
ebc_info = {i[0]: i[1] for i in pd.read_csv(ebc_file).as_matrix(['Environment.BC', 'Putative.Environment'])}
...
for b in bfas:
    ...
    dats[b]['Putative.Environment'] = dats[b].apply(lambda row: ebc_info[row['Environment.BC']], axis=1)
